...
\large —— 2025 CUMCM 一等奖论文复现报告}
\author{胡博文\quad 王天希\quad 夏泽宇}
\date{\today}
...
\begin{abstract}
本文围绕“板凳龙”在盘入、调头与盘出过程中的运动状态及碰撞风险问题，依据所选一等奖论文的建模思路，对其核心假设、几何结构、运动学关系与数值求解过程进行复现。文中先对题意与五个子问题作重新梳理，再在刚体近似、等速运动与路径连续等假设下，建立各节把手的位置、速度及相对距离模型；随后分别讨论碰撞判定、最小螺距估计、调头路径构造和最大行进速度分析；最后结合数值结果对模型的合理性与局限性进行说明。
...
本文以所选一等奖论文为复现对象，重点整理其在“板凳龙”运动轨迹、碰撞检测、调头路径构造以及速度上界分析中的建模思路、数学表达与求解流程。
...
因此，复现工作的重点不仅在于得到数值结果，更在于理清以下建模链条：
\[
...
\subsection{各问题之间的联系}
问题一提供后续分析所需的基础位置与速度信息；问题二在问题一的基础上增加板凳宽度因素并进行几何碰撞判定；问题三进一步把碰撞约束转化为路径参数的优化问题；问题四将路径从单一螺线扩展为“盘入--调头--盘出”的组合曲线；问题五则在全流程运动模型基础上研究速度边界。因此五个问题之间具有清晰的递进关系。

% ==================== 3 模型假设 ====================
\section{模型假设}

为便于建立后续模型，并与原论文处理方式保持一致，作如下假设：
...
% ==================== 8 AI使用声明 ====================
\section{AI 工具使用声明}
本文在写作与排版过程中适度使用了大语言模型，主要用于 LaTeX 排版辅助、文字修改建议以及个别段落的表述整理。涉及模型设定、公式推导、结果核验与最终定稿的工作均由作者独立完成，并对全文内容负责。

% ==================== 参考文献 ====================
\begin{thebibliography}{99}
\bibitem{ref1}
《基于动态搜索的“板凳龙”运动状态及路线研究》, 2025 年 CUMCM 一等奖论文.
\end{thebibliography}
...
\section{程序说明}
本附录给出程序结构说明及部分实现片段，用于说明主要计算流程与复现方法。

\section{附图与补充表格}
下列代码仅为核心实现的节选，完整程序、数据与运行说明可通过下述链接获取。

\noindent 完整材料链接：\url{https://github.com/dsa21ewq/mm_project1}

\subsection{核心代码片段（problem1.py，节选）}
\paragraph{轨迹积分与把手递推（以下代码为节选）}
...
\subsection{性能优化与统一配置（project\_refactor\_improved.py，节选）}
\paragraph{统一配置与快速根求解（以下代码为节选）}
